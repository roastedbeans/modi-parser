...
static void init_proto_names (const char *protos []) {
    ...
    protos[415] = "nr-rrc.radio_bearer_config";
    protos[416] = "nas-5gs";
}
    ...
    for (int i = 0; i <= PROTO_MAX; i++) {
        if (protos[i] != NULL) {
            ...
            if (handle != NULL) {
                ...
                fprintf(stderr, "Warning: dissector '%s' (ID %d) not found, skipping\n", protos[i], i);
            }
        }
    }
}
...
void proto_register_aww() {
    printf("Android stub: proto_register_aww()\n");
}

void proto_reg_handoff_aww() {
    printf("Android stub: proto_reg_handoff_aww()\n");
}

void print_proto_list() {
    printf("Android stub: Limited protocol list\n");
    printf("0: android_stub\n");
}
