...
class DataWriter:
    ...
    def __init__(self):
        ...
        # Load RRC and NAS headers for field mapping
        self.rrc_headers = self._load_rrc_headers()
        self.nas_headers = self._load_nas_headers()

    def _load_rrc_headers(self):
        """Load RRC headers from JSON file"""
        try:
            rrc_headers_path = os.path.join(os.path.dirname(__file__), 'rrc_headers.json')
            if os.path.exists(rrc_headers_path):
                with open(rrc_headers_path, 'r', encoding='utf-8') as f:
                    return json.load(f)
            else:
                self.logger.warning("RRC headers JSON file not found, using basic field mapping")
                return None
        except Exception as e:
            self.logger.warning(f"Failed to load RRC headers: {e}")
            return None

    def _load_nas_headers(self):
        """Load NAS headers from JSON file"""
        try:
            nas_headers_path = os.path.join(os.path.dirname(__file__), 'nas_headers.json')
            if os.path.exists(nas_headers_path):
                with open(nas_headers_path, 'r', encoding='utf-8') as f:
                    return json.load(f)
            else:
                self.logger.warning("NAS headers JSON file not found, using basic field mapping")
                return None
        except Exception as e:
            self.logger.warning(f"Failed to load NAS headers: {e}")
            return None

    def _get_rrc_field_mapping(self, protocol_type):
        """Get field mapping for RRC protocol type (lte_rrc or nr_rrc)"""
        if not self.rrc_headers:
            return {}
        
        field_mapping = {}
        if protocol_type in self.rrc_headers.get('fields', {}):
            protocol_fields = self.rrc_headers['fields'][protocol_type]
            for category, fields in protocol_fields.items():
                for field in fields:
                    ...
                    # Create mapping from clean name to field info
                    field_mapping[clean_name] = {
    ...
    def _get_nas_field_mapping(self, protocol_type):
        """Get field mapping for NAS protocol type (nas_eps, gsm_map, e212)"""
        if not self.nas_headers:
            return {}
        
        field_mapping = {}
        if protocol_type in self.nas_headers.get('fields', {}):
            protocol_fields = self.nas_headers['fields'][protocol_type]
            
            # Handle nested structure for nas_eps
            if isinstance(protocol_fields, dict):
                ...
                # Handle flat structure for gsm_map, e212, etc.
    ...
    def _add_lte_rrc_fields(self, proto_elem, layer):
        """Add specific LTE RRC fields with proper structure using JSON mapping"""
        try:
            # Get LTE RRC field mapping
            lte_field_mapping = self._get_rrc_field_mapping('lte_rrc')
            ...
            # Process all available fields from the layer
            field_names = layer.field_names if hasattr(layer, 'field_names') else []
            ...
            for field_name in field_names:
                try:
                    ...
                    if field_value is not None:
                        ...
                        # Get field info from mapping if available
                        if field_name in lte_field_mapping:
                            field_info = lte_field_mapping[field_name]
                            field_elem.set("showname", f"{field_info['description']}: {field_value}")
                            field_elem.set("field_type", field_info['field_type'])
                            field_elem.set("category", field_info['category'])
                        else:
                            field_elem.set("showname", f"{field_name}: {field_value}")
    ...
    def _add_nr_rrc_fields(self, proto_elem, layer):
        """Add specific 5G NR RRC fields using JSON mapping"""
        try:
            # Get NR RRC field mapping
            nr_field_mapping = self._get_rrc_field_mapping('nr_rrc')
            ...
            for field_name in field_names:
                try:
                    ...
                    if field_value is not None:
                        ...
                        # Get field info from mapping if available
                        if field_name in nr_field_mapping:
                            field_info = nr_field_mapping[field_name]
                            field_elem.set("showname", f"{field_info['description']}: {field_value}")
                            field_elem.set("field_type", field_info['field_type'])
                            field_elem.set("category", field_info['category'])
                        else:
    ...
    def _add_nas_fields(self, proto_elem, layer):
        """Add specific NAS fields using JSON mapping"""
        try:
            # Get NAS field mappings
            nas_eps_mapping = self._get_nas_field_mapping('nas_eps')
            gsm_map_mapping = self._get_nas_field_mapping('gsm_map')
            e212_mapping = self._get_nas_field_mapping('e212')
            ...
            for field_name in field_names:
                try:
                    ...
                    if field_value is not None:
                        ...
                        # Determine protocol prefix and mapping
                        if field_name.startswith('nas_eps_'):
                            protocol_prefix = 'nas-eps'
                            field_mapping = nas_eps_mapping
                        elif field_name.startswith('gsm_'):
                            protocol_prefix = 'gsm-map'
                            field_mapping = gsm_map_mapping
                        elif field_name.startswith('e212_'):
                            protocol_prefix = 'e212'
                            field_mapping = e212_mapping
                        else:
                            protocol_prefix = 'nas-eps'
                            field_mapping = nas_eps_mapping
                        
                        # Use original field name for compatibility
                        field_elem.set("name", f"{protocol_prefix}.{field_name}")
                        
                        # Get field info from mapping if available
                        clean_field_name = self._clean_field_name_for_mapping(field_name)
                        if clean_field_name in field_mapping:
                            field_info = field_mapping[clean_field_name]
                            field_elem.set("showname", f"{field_info['description']}: {field_value}")
                            field_elem.set("field_type", field_info['field_type'])
                            field_elem.set("category", field_info['category'])
                        else:
    ...
    def _clean_field_name_for_mapping(self, field_name):
        """Clean field name for mapping lookup"""
